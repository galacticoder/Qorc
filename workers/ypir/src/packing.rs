...
pub fn fast_multiply_no_reduce(
    ...
    unsafe {
        ...
        for idx in (0..pol_sz).step_by(8) {
            let mut sum_lo = crate::m512::zero();
            let mut sum_hi = crate::m512::zero();
            for k in 0..a.cols {
                let p_x = a_ptr.add(k * 2 * pol_sz + idx);
                let p_y = b_ptr.add(k * 2 * pol_sz + idx);

                let x = crate::m512::load(p_x as *const u64);
                let x_lo = x;
                let x_hi = crate::m512::srli32(x);
                let y = crate::m512::load(p_y as *const u64);
                let y_lo = y;
                let y_hi = crate::m512::srli32(y);

                let product_lo = crate::m512::mul_lo32(x_lo, y_lo);
                let product_hi = crate::m512::mul_lo32(x_hi, y_hi);

                sum_lo = crate::m512::add64(sum_lo, product_lo);
                sum_hi = crate::m512::add64(sum_hi, product_hi);
            }

            let p_z = res_ptr.add(idx);
            crate::m512::store(p_z as *mut u64, sum_lo);
            let p_z = res_ptr.add(pol_sz + idx);
            crate::m512::store(p_z as *mut u64, sum_hi);
        }
    }
}
...
pub fn multiply_add_poly_avx(_params: &Params, res: &mut [u64], a: &[u64], b: &[u64]) {
    unsafe {
        ...
        for i in (0..res.len()).step_by(8) {
            ...
            let x = crate::m512::load(p_x as *const u64);
            let y = crate::m512::load(p_y as *const u64);
            let z = crate::m512::load(p_z as *const u64);

            let product = crate::m512::mul_lo32(x, y);
            let out = crate::m512::add64(z, product);

            crate::m512::store(p_z as *mut u64, out);
        }
    }
}
...
pub fn multiply_poly_avx(_params: &Params, res: &mut [u64], a: &[u64], b: &[u64]) {
    unsafe {
        ...
        for i in (0..res.len()).step_by(8) {
            ...
            crate::m512::store(p_z as *mut u64, product);
        }
    }
}
