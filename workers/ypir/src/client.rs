...
// Qor patch: decouple the `&mut Client` borrow lifetime ('b) from the params lifetime ('a). The
// original `inner: &'a mut Client<'a>` forced the mutable borrow to last as long as params, which
// prevented a caller from holding a `Client` after building a query (the YPIR client daemon needs to
// keep the Client between query-record and recover-record). 'b is a short, transient borrow.
pub struct YClient<'b, 'a> {
    inner: &'b mut Client<'a>,
    params: &'a Params,
...
impl<'b, 'a> YClient<'b, 'a> {
    pub fn new(inner: &'b mut Client<'a>, params: &'a Params) -> Self {
        Self {
            inner,
            params,
