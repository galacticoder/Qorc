    ...
    *const T: ToM512,
{
    ...
    // Qor patch: return tied to the params lifetime `'a`, not the elided `&self` lifetime. The
    // returned value's data is owned (hint_0/1) or borrows params (PolyMatrixNTT<'a>, the params-built
    // smaller_server) — it does NOT borrow `&self`/the DB — so this lets a long-lived worker hold the
    // built YServer and its OfflinePrecomputedValues in one struct (see tier2::Tier2Worker).
