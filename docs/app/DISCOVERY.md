...
Because HintlessPIR cost scales with record SIZE, the tier-1 discovery record is a tiny 24-byte handle, not the inlined bundle — which is what lets the discovery database grow to millions of records at sub-second query time. The full post-quantum keys-blob (about 131 KB) is then delivered by a **k-anonymous bucket fetch** rather than PIR: the keys-blob is too large for a single-server PIR (the SimplePIR DB is `2048 rows × blobLen` and the build OOMs above about 64 KB records — an oblivious YPIR tier-2 was prototyped and abandoned for this reason). The bucket fetch trades full obliviousness for K-anonymity (the server learns the target's bucket = one of K=32 users, never the exact user), which is the same privacy class already used for avatars. The keys-blob content is unchanged.
