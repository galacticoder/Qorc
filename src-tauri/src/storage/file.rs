...
fn invalid_private_path(message: &str) -> QorcError {
    QorcError::FileOperationFailed(message.to_string())
}

pub fn validate_private_file_metadata(metadata: &std::fs::Metadata) -> QorcResult<()> {
...
pub async fn check_dir(path: &Path, mode: u32) -> QorcResult<()> {
...
pub async fn atomic_write(path: &Path, data: &[u8], mode: u32) -> QorcResult<()> {
    let parent = path.parent().ok_or_else(|| {
        QorcError::FileOperationFailed("Cannot determine parent directory".to_string())
    })?;
...
pub async fn atomic_write_if_absent(path: &Path, data: &[u8], mode: u32) -> QorcResult<bool> {
    let parent = path.parent().ok_or_else(|| {
        QorcError::FileOperationFailed("Cannot determine parent directory".to_string())
    })?;
    check_dir(parent, 0o700).await?;
...
pub async fn read_file_bounded(path: &Path, max_bytes: usize) -> QorcResult<Vec<u8>> {
...
/// Remove file
pub async fn remove_file(path: &Path) -> QorcResult<()> {
    match fs::symlink_metadata(path).await {
        Ok(metadata) => {
            validate_private_file_metadata(&metadata)?;
...
pub async fn private_file_exists(path: &Path) -> QorcResult<bool> {
    match fs::symlink_metadata(path).await {
        Ok(metadata) => {
            validate_private_file_metadata(&metadata)?;
            Ok(true)
...
pub async fn remove_stale_temp_files(dir: &Path) -> QorcResult<()> {
...
mod tests {
    ...
    async fn no_clobber_write_keeps_one_concurrent_winner() {
        let dir = std::env::temp_dir().join(format!("qorc-storage-test-{}", uuid::Uuid::new_v4()));
