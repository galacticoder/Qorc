...
// YPIR tier-2 client daemon

fn ypir_binary_names() -> Vec<&'static str> {
    if cfg!(windows) {
        vec!["ypir-client.exe"]
    } else {
        vec!["ypir-client"]
...
fn find_ypir_client_binary(app: &AppHandle) -> Result<PathBuf, String> {
    let mut paths: Vec<PathBuf> = Vec::new();
    if let Ok(raw) = std::env::var("QOR_YPIR_CLIENT_BIN") {
        let trimmed = raw.trim();
        if !trimmed.is_empty() {
            paths.push(PathBuf::from(trimmed));
        }
    ...
    Err("Local YPIR client binary not found".to_string())
}
...
static YPIR_DAEMON: OnceLock<tokio::sync::Mutex<Option<YpirDaemon>>> = OnceLock::new();

fn ypir_daemon_slot() -> &'static tokio::sync::Mutex<Option<YpirDaemon>> {
    YPIR_DAEMON.get_or_init(|| tokio::sync::Mutex::new(None))
}

fn spawn_ypir_daemon(app: &AppHandle) -> Result<YpirDaemon, String> {
    let binary = find_ypir_client_binary(app)?;
    let mut child = Command::new(binary)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .kill_on_drop(true)
        .spawn()
        .map_err(|e| format!("Failed to start YPIR client daemon: {e}"))?;
    let stdin = child
        .stdin
        .take()
        .ok_or_else(|| "YPIR client daemon stdin unavailable".to_string())?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "YPIR client daemon stdout unavailable".to_string())?;
    Ok(YpirDaemon {
        child,
        stdin,
        stdout: tokio::io::BufReader::new(stdout),
...
async fn ypir_daemon_exchange(daemon: &mut YpirDaemon, request: &Value) -> Result<Value, String> {
    ...
    daemon
        ...
        .await
        .map_err(|e| format!("ypir_daemon_write_failed: {e}"))?;
    daemon
        .stdin
        .flush()
        .await
        .map_err(|e| format!("ypir_daemon_flush_failed: {e}"))?;
    let mut resp = String::new();
    let n = daemon
        .stdout
        .read_line(&mut resp)
        .await
        .map_err(|e| format!("ypir_daemon_read_failed: {e}"))?;
    if n == 0 {
        return Err("ypir_daemon_eof".to_string());
    }
    serde_json::from_str(resp.trim()).map_err(|e| format!("ypir_daemon_bad_json: {e}"))
}

fn ypir_str_field(v: &Value, key: &str) -> Result<String, String> {
    v.get(key)
        .and_then(|x| x.as_str())
        .map(|s| s.to_string())
        .ok_or_else(|| format!("ypir_missing_field:{key}"))
}

/// fetch the discovery blob at tier 1 slot thru YPIR
#[tauri::command]
pub async fn ypir_fetch_blob(
    app: AppHandle,
    state: State<'_, AppState>,
    slot: usize,
) -> Result<Value, String> {
    // query record lock only for the pipe exchange
    let (handle, request) = {
        let mut guard = ypir_daemon_slot().lock().await;
        if guard.is_none() {
            *guard = Some(spawn_ypir_daemon(&app)?);
        }
        let daemon = guard.as_mut().unwrap();
        let q = ypir_daemon_exchange(
            daemon,
            &serde_json::json!({ "op": "query-record", "slot": slot }),
        )
        .await?;
        (
            ypir_str_field(&q, "handle")?,
            ypir_str_field(&q, "request")?,
        )
    };

    // Network POST
    let resp = isolated_tor_post(
        state.inner(),
        "ypir/query",
    ...
    .await?;
    let response = ypir_str_field(&resp, "response")?;

    // recover record reacquire lock for pipe exchange
    let blob = {
        let mut guard = ypir_daemon_slot().lock().await;
        let daemon = guard
            .as_mut()
            .ok_or_else(|| "ypir_daemon_gone".to_string())?;
        let r = ypir_daemon_exchange(
            daemon,
            &serde_json::json!({ "op": "recover-record", "handle": handle, "response": response }),
        )
        .await?;
        ypir_str_field(&r, "blob")?
    };
    Ok(serde_json::json!({ "ok": true, "blob": blob }))
}
