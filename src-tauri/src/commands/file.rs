...
pub async fn file_get_download_settings(
    ...
    let ask_where = storage
        .get("ask_where_to_save")
        .await
        .map_err(|e| e.safe_message())?
        .map(|s| s == "true")
        .unwrap_or(false);
...
#[tauri::command]
pub async fn file_choose_download_path(app: AppHandle) -> Result<Option<String>, String> {
...
/// Read file as base64
#[tauri::command]
pub async fn file_read_base64(path: String) -> Result<String, String> {
    let path_buf = PathBuf::from(&path);
...
/// Get file info
#[tauri::command]
pub async fn file_get_info(path: String) -> Result<FileInfo, String> {
    let path_buf = PathBuf::from(&path);
