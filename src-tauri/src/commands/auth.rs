...
pub async fn device_get_credentials(
    ...
    let existing_key = storage
        .get("device_mldsa_public_key")
        .await
        .map_err(|e| e.safe_message())?;
    let existing_hash = storage
        .get("device_mldsa_key_hash")
        .await
        .map_err(|e| e.safe_message())?;
    ...
    let key_hash_b64 =
        base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &key_hash);
    ...
    storage
        ...
        .map_err(|e| e.safe_message())?;
    storage
        .set("device_mldsa_public_key", &public_key_b64)
        .await
        .map_err(|e| e.safe_message())?;
    storage
        .set("device_mldsa_key_hash", &key_hash_b64)
        .await
        .map_err(|e| e.safe_message())?;
    storage
        .set("device_mldsa_secret_key", &secret_key_b64)
        .await
        .map_err(|e| e.safe_message())?;
...
pub async fn device_sign_challenge(
    ...
    let secret_key_b64 = storage
        .get("device_mldsa_secret_key")
        .await
        .map_err(|e| e.safe_message())?
        .ok_or_else(|| "No device secret key".to_string())?;

    let secret_bytes =
        base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &secret_key_b64)
            .map_err(|e| format!("Invalid secret key: {}", e))?;

    // Sign challenge using Dilithium-87
    let signature_bytes =
        crate::crypto::post_quantum::dilithium_sign(challenge.as_bytes(), &secret_bytes)
