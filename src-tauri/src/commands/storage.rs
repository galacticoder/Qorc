...
pub async fn secure_init(
    ...
    crate::storage::init(&state, config_dir)
        .await
        .map(|_| true)
...
#[tauri::command]
pub async fn secure_keys(state: State<'_, AppState>) -> Result<Vec<String>, String> {
    let storage = state
        .inner()
        .storage()
        .ok_or_else(|| "Storage not initialized".to_string())?;

    storage.keys().await.map_err(|e| e.safe_message())
}

/// Clear all secure storage
#[tauri::command]
pub async fn secure_clear(state: State<'_, AppState>) -> Result<bool, String> {
    let storage = state
        .inner()
        .storage()
        .ok_or_else(|| "Storage not initialized".to_string())?;
